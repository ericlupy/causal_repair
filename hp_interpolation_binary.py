...
def interpolate_binary(factual: np.array, counterfactual: np.array, step_ctrl=0.1):
    ...
    for x1 in range(factual.shape[0]):
        for x2 in range(factual.shape[1]):
            ...
            # midpoint
            mid = (solution[x1][x2] + factual[x1][x2]) / 2

            # no need to interpolate
            if abs(mid - factual[x1][x2]) <= step_ctrl / 4:
                continue
            else:

                temp_solution = np.copy(solution)

                while abs(mid - factual[x1][x2]) > step_ctrl / 4:

                    # interpolation step
                    start_interpolation = time.time()
                    temp_solution[x1][x2] = mid
                    mid = (temp_solution[x1][x2] + factual[x1][x2]) / 2  # approaching factual
...
if __name__ == '__main__':
    ...
    parser.add_argument("--factual_ctrl_name", help="approx broken controller (npy) computed in hp_gridding",
                        default='sig_8x16.npy')
    parser.add_argument("--counterfactual_ctrl_name",
                        help="sampled good controller (npy) computed in hp_sample_controller",
    ...
    f_solution, total_time, total_simulator_time, total_interpolation_time, num_simulator_runs, num_interpolation_steps = interpolate_binary(f_factual, f_counterfactual)
